...
cell_feature_normalized = np.load(
    rpath + f"Data/Cell/cell_feature_std_2369disjoint.npy", allow_pickle=True
).item()
...
def computing_sim_matrix():
    ...
    """
    if os.path.exists(dict_dir + "cell_sim_matrix") and os.path.exists(
        dict_dir + "drug_sim_matrix"
    ):
    ...
    for i in range(len(cell_id2idx_dict)):
        for j in range(len(cell_id2idx_dict)):
            if i != j:
                cell_sim_matrix[i, j], _ = pearsonr(
                    cell_feature_normalized[i], cell_feature_normalized[j]
                )
