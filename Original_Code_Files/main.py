...
def arg_parse():
    ...
    parser.add_argument(
        "--dropout_ratio", type=float, default=0.1, help="Dropout ratio (default: 0.1)"
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    parser.add_argument(
        "--edge", type=str, default="STRING", help="STRING, BIOGRID"
    )  # BIOGRID: removed
    parser.add_argument(
        "--string_edge",
        type=float,
...
def main():
    ...
    drug_dict = np.load(
        rpath + "Data/Drug/drug_feature_graph.npy", allow_pickle=True
    ).item()  # pyg format of drug graph
    cell_dict = np.load(
        rpath + f"Data/Cell/cell_feature_std_{args.dataset}.npy", allow_pickle=True
