...
def train(model, loader, loss_fn, opt, args):
    ...
    for data in tqdm(loader, desc="Iteration"):
        ...
        if isinstance(cell, list):
            drug, cell, label = (
                drug.to(device),
                [feat.to(device) for feat in cell],
                label.to(device),
            )
        # single omics
        else:
...
def gradcam(model, drug_name, cell_name, drug_dict, cell_dict, edge_index, args):
    ...
    cell_node, cell_representation = model.CellEncoder.grad_cam(
        cell
    )  # cell node: torch.Size([4646, 8]), cell_representation: torch.Size([1, 37168])
    ...
    cell_representation = (
        model.cell_emb(model.padding(cell_representation, mask))
        if model.trans
        else model.cell_emb(cell_representation)
    )

    # combine drug feature and cell line feature
    x, _ = model.aggregate(
        cell_representation, drug_representation, trans=model.trans
...
def draw_pair_heatmap(attn_score, drug_name, cell_name, ticks, args):
    ...
    ax.set_yticklabels(
        ticks, fontsize=6
    )  # Due to 'invert_yaxis', yticklables can be flipped without 'ticks.reverse()'
...
def inference(model, drug_dict, cell_dict, edge_index, save_name, args):
    ...
    val_set, ntest_set = train_test_split(val_test_set, test_size=0.5, random_state=args.seed)

    cell_table = IC[["DepMap_ID", "stripped_cell_line_name"]].drop_duplicates(keep="first")
    drug_table = (
        IC["Drug name"].drop_duplicates(keep="first").to_frame()
    )  # to_frame(): Convert series to dataframe
    cell_table[
        "value"
    ] = 1  # Temporary variable for obtaining every combination of cell and drug according to the value
    drug_table["value"] = 1
    ...
    for dataset_name, data in dataset.items():
        ...
        IC50_pred = []
        with torch.no_grad():
            drug_name, cell_ID, cell_line_name = (
                data["Drug name"],
                data["DepMap_ID"],
            ...
            for index in range(batch_num):
                drug = Batch.from_data_list(
                    drug_list[index * batch_size : (index + 1) * batch_size]
                ).to(args.device)
                cell = Batch.from_data_list(
                    cell_list[index * batch_size : (index + 1) * batch_size]
                ).to(args.device)
        ...
        if dataset_name != "unknown":
            table["Abs_error"] = np.abs(
                IC50_pred.cpu().numpy() - np.array(table["IC50"]).reshape(-1, 1)
            )
...
def load_data(rs_tr, rs_te, rs_vl, drug_dict, cell_dict, edge_index, args):  # For PPI network
    ...
    Dataset = MyDataset
    collate_fn = _collate
    train_dataset = Dataset(drug_dict, cell_dict, rs_tr, edge_index=edge_index)
    val_dataset = Dataset(drug_dict, cell_dict, rs_vl, edge_index=edge_index)
    test_dataset = Dataset(drug_dict, cell_dict, rs_te, edge_index=edge_index)
    ...
    val_loader = DataLoader(
        val_dataset, batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn, num_workers=4
    )
    ...
    def __init__(self, mode="higher", patience=10, filename=None, metric=None):
        ...
        if metric is not None:
            assert metric in ["r2", "mae", "rmse", "roc_auc_score", "pr_auc_score"], (
                "Expect metric to be 'r2' or 'mae' or " f"'rmse' or 'roc_auc_score', got {metric}"
            )
...
class Dataset_Sim(Dataset):
    ...
    def __getitem__(self, index):
        return (
            drug_name2idx_dict[self.drug_name[index]],
            cell_id2idx_dict[self.Cell_line_name[index]],
            self.value[index],
        )
...
def load_sim_data(IC, args):
    ...
    train_data, val_data, test_data = (
        Dataset_Sim(train_set),
        Dataset_Sim(val_set),
        Dataset_Sim(test_set),
    )
...
def load_sim_graph(edge_index, args):
    ...
    cell_name2feature_dict = np.load(
        "Data/Cell/cell_feature_std_2369disjoint.npy", allow_pickle=True
    ).item()
    ...
    model.load_state_dict(
        torch.load(f"weights/weight_seed{args.seed}.pth", map_location=args.device)[
            "model_state_dict"
        ]
    )

    drug_nodes = model.DrugEncoder(
        Batch.from_data_list(drug_graph).to(args.device)
    ).detach()  # detach(): One of the ways to copy an existing tensor - create a tensor that does not allow gradient propagation from an existing tensor
    cell_nodes = model.CellEncoder(
