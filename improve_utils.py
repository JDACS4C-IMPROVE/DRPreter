...
improve_globals.x_data_dir_name = "data.ccle/x_data"  # feature data
improve_globals.y_data_dir_name = "data.ccle/y_data"  # target data
improve_globals.splits_dir_name = "data.ccle/splits"  # splits files
...
improve_globals.canc_col_name = (
    "CancID"  # column name that contains the cancer sample ids TODO: rename to sample_col_name
)
improve_globals.drug_col_name = "DrugID"  # column name that contains the drug ids
improve_globals.source_col_name = (
    "source"  # column name that contains source/study names (CCLE, GDSCv1, etc.)
)
improve_globals.pred_col_name_suffix = (
    "_pred"  # suffix to predictions col name (example of final col name: auc_pred)
)

# Response data file name
improve_globals.y_file_name = "rsp_full.csv"  # response data

# Cancer sample features file names
improve_globals.copy_number_fname = "cancer_copy_number.tsv"  # cancer feature
improve_globals.discretized_copy_number_fname = (
    "cancer_discretized_copy_number.tsv"  # cancer feature
)
improve_globals.dna_methylation_fname = "cancer_DNA_methylation.tsv"  # cancer feature
improve_globals.gene_expression_fname = "ge.csv"  # cancer feature
...
improve_globals.raw_data_dir = (
    improve_globals.main_data_dir / improve_globals.raw_data_dir_name
)  # raw_data
improve_globals.ml_data_dir = (
    improve_globals.main_data_dir / improve_globals.ml_data_dir_name
)  # ml_data
improve_globals.models_dir = (
    improve_globals.main_data_dir / improve_globals.models_dir_name
)  # models
improve_globals.infer_dir = improve_globals.main_data_dir / improve_globals.infer_dir_name  # infer
# -----
improve_globals.x_data_dir = (
    improve_globals.raw_data_dir / improve_globals.x_data_dir_name
)  # x_data
improve_globals.y_data_dir = (
    improve_globals.raw_data_dir / improve_globals.y_data_dir_name
)  # y_data
improve_globals.splits_dir = (
    improve_globals.raw_data_dir / improve_globals.splits_dir_name
)  # splits

# Response
improve_globals.y_file_path = (
    improve_globals.y_data_dir / improve_globals.y_file_name
)  # response.txt

# Cancers
improve_globals.copy_number_file_path = (
    improve_globals.x_data_dir / improve_globals.copy_number_fname
)  # cancer_copy_number.txt
improve_globals.discretized_copy_number_file_path = (
    improve_globals.x_data_dir / improve_globals.discretized_copy_number_fname
)  # cancer_discretized_copy_number.txt
improve_globals.dna_methylation_file_path = (
    improve_globals.x_data_dir / improve_globals.dna_methylation_fname
)  # cancer_DNA_methylation.txt
improve_globals.gene_expression_file_path = (
    improve_globals.x_data_dir / improve_globals.gene_expression_fname
)  # cancer_gene_expression.txt
improve_globals.mirna_expression_file_path = (
    improve_globals.x_data_dir / improve_globals.miRNA_expression_fname
)  # cancer_miRNA_expression.txt
improve_globals.mutation_count_file_path = (
    improve_globals.x_data_dir / improve_globals.mutation_count_fname
)  # cancer_mutation_count.txt
improve_globals.mutation_file_path = (
    improve_globals.x_data_dir / improve_globals.mutation_fname
)  # cancer_mutation.txt
improve_globals.rppa_file_path = (
    improve_globals.x_data_dir / improve_globals.rppa_fname
)  # cancer_RPPA.txt

# Drugs
improve_globals.smiles_file_path = improve_globals.x_data_dir / improve_globals.smiles_file_name  #
improve_globals.mordred_file_path = (
    improve_globals.x_data_dir / improve_globals.mordred_file_name
)  #
improve_globals.ecfp4_512bit_file_path = (
    improve_globals.x_data_dir / improve_globals.ecfp4_512bit_file_name
)  #
# -----------------------------------------------------------------------------
...
def load_split_file(
    source: str, split: Union[int, None] = None, split_type: Union[str, List[str], None] = None
) -> List[int]:
    """
    Args:
...
def set_col_names_in_multilevel_dataframe(
    df: pd.DataFrame, level_map: dict, gene_system_identifier: Union[str, List[str]] = "Gene_Symbol"
) -> pd.DataFrame:
    ...
    if isinstance(gene_system_identifier, str):
        if gene_system_identifier == "all":
            ...
            )  # retian specific column level
    else:
        assert (
            len(gene_system_identifier) <= n_levels
        ), f"'gene_system_identifier' can't contain more than {n_levels} items."
...
def load_discretized_copy_number_data(
    ...
    df = pd.read_csv(
        improve_globals.discretized_copy_number_file_path, sep=sep, index_col=0, header=header
    )
...
def load_smiles_data(sep: str = ",", verbose: bool = True) -> pd.DataFrame:
    """
...
def load_mordred_descriptor_data(sep: str = "\t", verbose: bool = True) -> pd.DataFrame:
    """
...
def load_morgan_fingerprint_data(sep: str = "\t", verbose: bool = True) -> pd.DataFrame:
    """
...
def save_preds(
    df: pd.DataFrame, y_col_name: str, outpath: Union[str, PosixPath], round_decimals: int = 4
) -> None:
    ...
    assert (
        improve_globals.canc_col_name in df.columns
    ), f"{improve_globals.canc_col_name} was not found in columns."
    assert (
        improve_globals.drug_col_name in df.columns
    ), f"{improve_globals.drug_col_name} was not found in columns."
...
# ==================================================================
def get_data_splits(
    src_raw_data_dir: str, splitdir_name: str, split_file_name: str, rsp_df: pd.DataFrame
):
    """
    IMPROVE-specific func.
    Read smiles data.
