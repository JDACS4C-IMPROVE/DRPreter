...
class DRPreter(nn.Module):
    def __init__(self, params):
        ...
        self.cum_num_nodes = params["cum_num_nodes"]
        self.max_gene = params["max_gene"]
        self.n_pathways = params["n_pathways"]

        self.layer_drug = params["layer_drug"]
        self.dim_drug = params["dim_drug"]
        self.layer_cell = params["layer"]
        self.dim_cell = params["hidden_dim"]
        self.dim_drug_cell = params["dim_drug_cell"]
        ...
        self.CellEncoder = CellEncoder(
            self.num_feature, self.num_genes, self.layer_cell, self.dim_cell
        )
        ...
        if self.trans:
            ...
            self.Transformer = Transformer(
                d_model=self.dim_drug_cell,
                nhead=8,
                num_encoder_layers=1,
                dim_feedforward=self.dim_drug_cell,
            )
            # reg_input = self.dim_drug_cell * (params["n_pathways"]+1)
    ...
    def padding(self, x, mask):
        ...
        x_pad = x.new_full(
            (x.size(0), (mask[-1].item() + 1) * self.max_gene, x.size(2)), 0
        )  # x_pad.shape: torch.Size([128, 11934, 8])
        cum_num_nodes = torch.tensor(self.cum_num_nodes)
        ...
        )  # index.shape: torch.Size([4646]) ===> index: tensor([   0,    1,    2,  ..., 4643, 4644, 4645], device='cuda:0')
        index = (
            (index - cum_num_nodes[mask]) + (mask * self.max_gene)
        )  # index.shape: torch.Size([4646]) ===> index: tensor([    0,     1,     2,  ..., 11682, 11683, 11684], device='cuda:0')
        x_pad[:, index, :] = x
        x_pad = x_pad.view(
            x.size(0), mask[-1].item() + 1, -1
        )  # x_pad.shape: torch.Size([128, 34, 2808])
    ...
    def aggregate(self, x_cell, x_drug, trans=False):
        if trans:
            ...
            x_cell = x[
                ...
                dim=1
            )  # x[:, :-1, :].shape: torch.Size([128, 34, 256]) ==> x_cell.shape: torch.Size([128, 256])
