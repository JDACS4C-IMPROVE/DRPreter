...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=1024):
        ...
        div_term = torch.exp(
            torch.arange(0, d_model, 2).float() * (-torch.log(torch.Tensor([10000])) / d_model)
        )  # [max_len / 2]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        Q = (
            self.W_Q(input_Q).view(batch, -1, self.n_heads, self.d_k).transpose(1, 2)
        )  # [batch, n_heads, len_q, d_k]
        K = (
            self.W_K(input_K).view(batch, -1, self.n_heads, self.d_k).transpose(1, 2)
        )  # [batch, n_heads, len_k, d_k]
        V = (
            self.W_V(input_V).view(batch, -1, self.n_heads, self.d_v).transpose(1, 2)
        )  # [batch, n_heads, len_v, d_v]
        ...
        ).contiguous()  # [batch, len_q, n_heads * d_v]
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, encoder_input, encoder_pad_mask):
        ...
        """
        encoder_output, attn = self.encoder_self_attn(
            encoder_input, encoder_input, encoder_input, encoder_pad_mask
        )
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                EncoderLayer(d_model, d_k, d_v, nhead, dim_feedforward, dropout)
    ...
    def forward(self, encoder_input):
        ...
        encoder_self_attn_mask = get_attn_pad_mask(
            encoder_input[:, :, 0], encoder_input[:, :, 0]
        )  # [batch, source_len, source_len]
